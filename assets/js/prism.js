/* PrismJS 1.23.0
https://prismjs.com/download.html#themes=prism-tomorrow&languages=markup+css+clike+javascript&plugins=command-line */
var _self =
    ...
    Prism.hooks.add("before-highlight", function (e) {
      ...
      if (!t.complete && e.code) {
        ...
        if (
          ...
          (p.test(n.className) || p.test(e.element.className))
        ) {
          var a = e.element.querySelector("." + d)
          a && a.remove()
          var s = e.code.split("\n")
      ...
      Prism.hooks.add("complete", function (e) {
        ...
        if (!t.complete) {
          ...
          p.test(e.element.className) &&
            (e.element.className = e.element.className.replace(p, " ")),
            p.test(a.className) || (a.className += " command-line")
          var r = t.numberOfLines || 0,
            s = c("data-prompt", "")
          if ("" !== s) n = f('<span data-prompt="' + s + '"></span>', r)
          else
            n = f(
              '<span data-user="' +
                c("data-user", "user") +
